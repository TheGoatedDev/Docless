...
- **Auto-rename (ADR 0009):** if `autoRename` and `auto_renamed=0`, OCR `done` enqueues a rename job (does not hold an OCR slot). Job sends full OCR text to `llama3.2:1b` for DATE/VENDOR/WHAT, assembles `YYYY-MM-DD-Vendor-What` (document date else mtime else omit; hyphens; keep ext; `-2`/`-3` on clash), `fs.rename` same folder. UPDATE path then rename; ignore watcher unlink/add for that move. Fail/missing model leaves the file and flag unset. Finder rename still = new path (re-OCR).
