...
- **Auto-rename (ADR 0009):** if `autoRename` and `auto_renamed=0`, after OCR `done` ask `llama3.2:1b` for a stem, slug it, `fs.rename` in the same folder (keep ext, `-2`/`-3` on clash). UPDATE path then rename; ignore watcher unlink/add for that move. Fail/missing model leaves the file and flag unset. Finder rename still = new path (re-OCR).
