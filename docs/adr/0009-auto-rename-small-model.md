...
After OCR succeeds, if `autoRename` is on, a separate queue asks `llama3.2:1b` for DATE/VENDOR/WHAT over the full OCR text and `fs.rename`s to `YYYY-MM-DD-Vendor-What.ext` (all three required; document date else mtime; hyphens). Skip if any part missing. Does not block the OCR pool. Sidecar path PK is updated first; watcher events for that move are ignored. Default off, future OCR only, once per row (`auto_renamed`). Not Finder rename-merge (ADR 0008). Still on-device, still a fixed tag (ADR 0003).
